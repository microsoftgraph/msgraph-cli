...
namespace ApiSdk.Users.Item.CalendarGroups.Item.Calendars.Item.Events.Item.Calendar.AllowedCalendarSharingRolesWithUser {
    ...
    public class AllowedCalendarSharingRolesWithUserRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string userId, string calendarGroupId, string calendarId, string eventId, string User, FormatterType output, IOutputFormatterFactory outputFormatterFactory, CancellationToken cancellationToken) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo, errorMapping: default, cancellationToken: cancellationToken);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response);
            }, userIdOption, calendarGroupIdOption, calendarIdOption, eventIdOption, UserOption, outputOption);
            return command;
        }
        /// <summary>
