...
namespace ApiSdk.Users.Item.Insights.Trending.Item.Resource.WorkbookRange.CellWithRowWithColumn {
    ...
    public class CellWithRowWithColumnRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string userId, string trendingItemId, int? row, int? column, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, userIdOption, trendingItemIdOption, rowOption, columnOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
