...
namespace ApiSdk.Users.Item.Insights.Shared.Item.Resource.WorkbookRange.ColumnWithColumn {
    ...
    public class ColumnWithColumnRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string userId, string sharedInsightId, int? column, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, userIdOption, sharedInsightIdOption, columnOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
