...
namespace ApiSdk.Users.Item.Insights.Shared.Item.Resource.WorkbookRange.ColumnsBeforeWithCount {
    ...
    public class ColumnsBeforeWithCountRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string userId, string sharedInsightId, int? count, FormatterType output, IOutputFormatterFactory outputFormatterFactory, CancellationToken cancellationToken) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo, errorMapping: default, cancellationToken: cancellationToken);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response);
            }, userIdOption, sharedInsightIdOption, countOption, outputOption);
            return command;
        }
        /// <summary>
