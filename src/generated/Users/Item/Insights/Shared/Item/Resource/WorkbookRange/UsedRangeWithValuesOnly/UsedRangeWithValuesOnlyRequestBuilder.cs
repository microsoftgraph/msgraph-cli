...
namespace ApiSdk.Users.Item.Insights.Shared.Item.Resource.WorkbookRange.UsedRangeWithValuesOnly {
    ...
    public class UsedRangeWithValuesOnlyRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string userId, string sharedInsightId, bool? valuesOnly, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, userIdOption, sharedInsightIdOption, valuesOnlyOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
