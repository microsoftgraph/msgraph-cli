...
namespace ApiSdk.Users.Item.MailFolders.Item.ChildFolders.Item.Move {
    ...
    public class MoveRequestBuilder {
        ...
        public Command BuildPostCommand() {
            ...
            command.SetHandler(async (string userId, string mailFolderId, string mailFolderId1, string body, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                using var stream = new MemoryStream(Encoding.UTF8.GetBytes(body));
                var parseNode = ParseNodeFactoryRegistry.DefaultInstance.GetRootParseNode("application/json", stream);
                var model = parseNode.GetObjectValue<MoveRequestBody>();
                var requestInfo = CreatePostRequestInformation(model, q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, userIdOption, mailFolderIdOption, mailFolderId1Option, bodyOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
