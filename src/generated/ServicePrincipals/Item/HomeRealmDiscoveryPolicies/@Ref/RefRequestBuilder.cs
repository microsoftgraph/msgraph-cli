    ...
    public class RefRequestBuilder {
        ...
        public Command BuildPostCommand() {
            ...
            command.Handler = CommandHandler.Create<string, string>(async (servicePrincipalId, body) => {
                ...
                var model = parseNode.GetObjectValue<ApiSdk.ServicePrincipals.Item.HomeRealmDiscoveryPolicies.@Ref.@Ref>();
                var requestInfo = CreatePostRequestInformation(model);
                if (!String.IsNullOrEmpty(servicePrincipalId)) requestInfo.PathParameters.Add("servicePrincipal_id", servicePrincipalId);
                var result = await RequestAdapter.SendAsync<ApiSdk.ServicePrincipals.Item.HomeRealmDiscoveryPolicies.@Ref.@Ref>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<ApiSdk.ServicePrincipals.Item.HomeRealmDiscoveryPolicies.@Ref.@Ref> PostAsync(ApiSdk.ServicePrincipals.Item.HomeRealmDiscoveryPolicies.@Ref.@Ref model, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default, CancellationToken cancellationToken = default) {
            _ = model ?? throw new ArgumentNullException(nameof(model));
            var requestInfo = CreatePostRequestInformation(model, h, o);
            return await RequestAdapter.SendAsync<ApiSdk.ServicePrincipals.Item.HomeRealmDiscoveryPolicies.@Ref.@Ref>(requestInfo, responseHandler, cancellationToken);
        }
