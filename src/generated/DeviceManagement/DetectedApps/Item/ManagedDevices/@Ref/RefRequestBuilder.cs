...
namespace ApiSdk.DeviceManagement.DetectedApps.Item.ManagedDevices.@Ref {
    ...
    public class RefRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.Handler = CommandHandler.Create<string, int?, int?, string, string, bool?, object>(async (detectedAppId, top, skip, search, filter, count, orderby) => {
                var requestInfo = CreateGetRequestInformation();
                if (!String.IsNullOrEmpty(detectedAppId)) requestInfo.PathParameters.Add("detectedApp_id", detectedAppId);
        ...
        public Command BuildPostCommand() {
            ...
            command.Handler = CommandHandler.Create<string, string>(async (detectedAppId, body) => {
                ...
                var model = parseNode.GetObjectValue<ApiSdk.DeviceManagement.DetectedApps.Item.ManagedDevices.@Ref.@Ref>();
                var requestInfo = CreatePostRequestInformation(model);
                if (!String.IsNullOrEmpty(detectedAppId)) requestInfo.PathParameters.Add("detectedApp_id", detectedAppId);
                var result = await RequestAdapter.SendAsync<ApiSdk.DeviceManagement.DetectedApps.Item.ManagedDevices.@Ref.@Ref>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<ApiSdk.DeviceManagement.DetectedApps.Item.ManagedDevices.@Ref.@Ref> PostAsync(ApiSdk.DeviceManagement.DetectedApps.Item.ManagedDevices.@Ref.@Ref model, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default, CancellationToken cancellationToken = default) {
            _ = model ?? throw new ArgumentNullException(nameof(model));
            var requestInfo = CreatePostRequestInformation(model, h, o);
            return await RequestAdapter.SendAsync<ApiSdk.DeviceManagement.DetectedApps.Item.ManagedDevices.@Ref.@Ref>(requestInfo, responseHandler, cancellationToken);
        }
