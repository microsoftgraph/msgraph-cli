...
namespace ApiSdk.Workbooks.Item.Workbook.TableRowOperationResultWithKey {
    ...
    public class TableRowOperationResultWithKeyRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string driveItemId, string key, FormatterType output, IOutputFormatterFactory outputFormatterFactory, CancellationToken cancellationToken) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo, errorMapping: default, cancellationToken: cancellationToken);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response);
            }, driveItemIdOption, keyOption, outputOption);
            return command;
        }
        /// <summary>
