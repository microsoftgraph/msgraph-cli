...
namespace ApiSdk.Workbooks.Item.Workbook.TableRowOperationResultWithKey {
    ...
    public class TableRowOperationResultWithKeyRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string driveItemId, string key, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, driveItemIdOption, keyOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
