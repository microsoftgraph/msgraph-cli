...
namespace ApiSdk.Workbooks.Item.Workbook.Tables.Item.Worksheet.Charts.Item.ImageWithWidthWithHeight {
    ...
    public class ImageWithWidthWithHeightRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string driveItemId, string workbookTableId, string workbookChartId, int? width, int? height, FormatterType output, IOutputFormatterFactory outputFormatterFactory, CancellationToken cancellationToken) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo, errorMapping: default, cancellationToken: cancellationToken);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response);
            }, driveItemIdOption, workbookTableIdOption, workbookChartIdOption, widthOption, heightOption, outputOption);
            return command;
        }
        /// <summary>
