...
namespace ApiSdk.Sites.Item.Onenote.Pages.Item.ParentNotebook.SectionGroups.Item.Sections.Item.Pages.Item.Preview {
    ...
    public class PreviewRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string siteId, string onenotePageId, string sectionGroupId, string onenoteSectionId, string onenotePageId1, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, siteIdOption, onenotePageIdOption, sectionGroupIdOption, onenoteSectionIdOption, onenotePageId1Option, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
