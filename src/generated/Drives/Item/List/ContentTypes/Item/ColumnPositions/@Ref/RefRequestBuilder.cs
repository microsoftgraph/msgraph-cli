    ...
    public class RefRequestBuilder {
        ...
        public Command BuildPostCommand() {
            ...
            command.Handler = CommandHandler.Create<string, string, string>(async (driveId, contentTypeId, body) => {
                ...
                if (!String.IsNullOrEmpty(driveId)) requestInfo.PathParameters.Add("drive_id", driveId);
                if (!String.IsNullOrEmpty(contentTypeId)) requestInfo.PathParameters.Add("contentType_id", contentTypeId);
                var result = await RequestAdapter.SendAsync<ApiSdk.Drives.Item.List.ContentTypes.Item.ColumnPositions.@Ref.@Ref>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<ApiSdk.Drives.Item.List.ContentTypes.Item.ColumnPositions.@Ref.@Ref> PostAsync(ApiSdk.Drives.Item.List.ContentTypes.Item.ColumnPositions.@Ref.@Ref model, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default) {
            _ = model ?? throw new ArgumentNullException(nameof(model));
            var requestInfo = CreatePostRequestInformation(model, h, o);
            return await RequestAdapter.SendAsync<ApiSdk.Drives.Item.List.ContentTypes.Item.ColumnPositions.@Ref.@Ref>(requestInfo, responseHandler);
        }
