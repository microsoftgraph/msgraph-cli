...
namespace ApiSdk.Drives.Item.List.ContentTypes.Item.Base.Ref {
    ...
    public class RefRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string driveId, string contentTypeId, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, driveIdOption, contentTypeIdOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
        /// Parent contentType from which this content type is derived.
