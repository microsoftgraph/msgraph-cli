...
namespace ApiSdk.Devices.Item.RegisteredOwners.@Ref {
    ...
    public class RefRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string deviceId, int? top, int? skip, string search, string filter, bool? count, string[] orderby) => {
            ...
            }, deviceIdOption, topOption, skipOption, searchOption, filterOption, countOption, orderbyOption);
            return command;
        }
        /// <summary>
        /// The user that cloud joined the device or registered their personal device. The registered owner is set at the time of registration. Currently, there can be only one owner. Read-only. Nullable. Supports $expand.
        ...
        public Command BuildPostCommand() {
            ...
            command.SetHandler(async (string deviceId, string body) => {
                ...
                var model = parseNode.GetObjectValue<ApiSdk.Devices.Item.RegisteredOwners.@Ref.@Ref>();
                var requestInfo = CreatePostRequestInformation(model, q => {
                });
                var result = await RequestAdapter.SendAsync<ApiSdk.Devices.Item.RegisteredOwners.@Ref.@Ref>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<ApiSdk.Devices.Item.RegisteredOwners.@Ref.@Ref> PostAsync(ApiSdk.Devices.Item.RegisteredOwners.@Ref.@Ref model, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default, CancellationToken cancellationToken = default) {
            _ = model ?? throw new ArgumentNullException(nameof(model));
            var requestInfo = CreatePostRequestInformation(model, h, o);
            return await RequestAdapter.SendAsync<ApiSdk.Devices.Item.RegisteredOwners.@Ref.@Ref>(requestInfo, responseHandler, cancellationToken);
        }
