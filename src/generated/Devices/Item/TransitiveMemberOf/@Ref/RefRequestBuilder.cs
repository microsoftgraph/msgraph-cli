    ...
    public class RefRequestBuilder {
        ...
        public Command BuildPostCommand() {
            ...
            command.Handler = CommandHandler.Create<string, string>(async (deviceId, body) => {
                ...
                if (!String.IsNullOrEmpty(deviceId)) requestInfo.PathParameters.Add("device_id", deviceId);
                var result = await RequestAdapter.SendAsync<ApiSdk.Devices.Item.TransitiveMemberOf.@Ref.@Ref>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<ApiSdk.Devices.Item.TransitiveMemberOf.@Ref.@Ref> PostAsync(ApiSdk.Devices.Item.TransitiveMemberOf.@Ref.@Ref model, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default) {
