...
namespace ApiSdk.AgreementAcceptances {
    ...
    public class AgreementAcceptancesRequestBuilder {
        ...
        public Command BuildListCommand() {
            ...
            command.Handler = CommandHandler.Create<string, object>(async (search, select) => {
                var requestInfo = CreateGetRequestInformation();
                if (!String.IsNullOrEmpty(search)) requestInfo.QueryParameters.Add("search", search);
                requestInfo.QueryParameters.Add("select", select);
                var result = await RequestAdapter.SendAsync<AgreementAcceptancesResponse>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
