...
namespace ApiSdk.Me.ReminderViewWithStartDateTimeWithEndDateTime {
    ...
    public class ReminderViewWithStartDateTimeWithEndDateTimeRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string StartDateTime, string EndDateTime, FormatterType output, IOutputFormatterFactory outputFormatterFactory, CancellationToken cancellationToken) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo, errorMapping: default, cancellationToken: cancellationToken);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response);
            }, StartDateTimeOption, EndDateTimeOption, outputOption);
            return command;
        }
        /// <summary>
