...
namespace ApiSdk.Me.Events.Item.Calendar.AllowedCalendarSharingRolesWithUser {
    ...
    public class AllowedCalendarSharingRolesWithUserRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string eventId, string User, FormatterType output, IOutputFormatterFactory outputFormatterFactory, CancellationToken cancellationToken) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo, errorMapping: default, cancellationToken: cancellationToken);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response);
            }, eventIdOption, UserOption, outputOption);
            return command;
        }
        /// <summary>
