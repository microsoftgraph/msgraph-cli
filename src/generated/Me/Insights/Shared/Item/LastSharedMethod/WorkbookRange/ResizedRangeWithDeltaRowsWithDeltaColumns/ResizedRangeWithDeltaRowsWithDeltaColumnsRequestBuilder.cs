...
namespace ApiSdk.Me.Insights.Shared.Item.LastSharedMethod.WorkbookRange.ResizedRangeWithDeltaRowsWithDeltaColumns {
    ...
    public class ResizedRangeWithDeltaRowsWithDeltaColumnsRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string sharedInsightId, int? deltaRows, int? deltaColumns, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, sharedInsightIdOption, deltaRowsOption, deltaColumnsOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
