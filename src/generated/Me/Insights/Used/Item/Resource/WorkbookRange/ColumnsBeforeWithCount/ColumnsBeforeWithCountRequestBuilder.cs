...
namespace ApiSdk.Me.Insights.Used.Item.Resource.WorkbookRange.ColumnsBeforeWithCount {
    ...
    public class ColumnsBeforeWithCountRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string usedInsightId, int? count, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, usedInsightIdOption, countOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
