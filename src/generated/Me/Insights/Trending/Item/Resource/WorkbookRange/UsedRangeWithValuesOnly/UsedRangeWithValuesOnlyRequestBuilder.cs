...
namespace ApiSdk.Me.Insights.Trending.Item.Resource.WorkbookRange.UsedRangeWithValuesOnly {
    ...
    public class UsedRangeWithValuesOnlyRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string trendingItemId, bool? valuesOnly, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, trendingItemIdOption, valuesOnlyOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
