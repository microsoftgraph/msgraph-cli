...
namespace ApiSdk.Me.Insights.Trending.Item.Resource.WorkbookRange.ColumnWithColumn {
    ...
    public class ColumnWithColumnRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string trendingItemId, int? column, FormatterType output, IOutputFormatterFactory outputFormatterFactory, CancellationToken cancellationToken) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo, errorMapping: default, cancellationToken: cancellationToken);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response);
            }, trendingItemIdOption, columnOption, outputOption);
            return command;
        }
        /// <summary>
