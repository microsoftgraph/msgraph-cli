...
namespace ApiSdk.Me.Insights.Trending.Item.Resource.PrintJob.Redirect {
    ...
    public class RedirectRequestBuilder {
        ...
        public Command BuildPostCommand() {
            ...
            command.SetHandler(async (string trendingItemId, string body, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                using var stream = new MemoryStream(Encoding.UTF8.GetBytes(body));
                var parseNode = ParseNodeFactoryRegistry.DefaultInstance.GetRootParseNode("application/json", stream);
                var model = parseNode.GetObjectValue<RedirectRequestBody>();
                var requestInfo = CreatePostRequestInformation(model, q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, trendingItemIdOption, bodyOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
