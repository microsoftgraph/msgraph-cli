...
namespace ApiSdk.Me.FollowedSites.@Ref {
    ...
    public class RefRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.Description = "Get ref of followedSites from me";
        ...
        public Command BuildPostCommand() {
            ...
            command.Description = "Create new navigation property ref to followedSites for me";
            // Create options for all the parameters
            command.AddOption(new Option<string>("--body"));
            command.Handler = CommandHandler.Create<string>(async (body) => {
                using var stream = new MemoryStream(Encoding.UTF8.GetBytes(body));
                var parseNode = ParseNodeFactoryRegistry.DefaultInstance.GetRootParseNode("application/json", stream);
                var model = parseNode.GetObjectValue<ApiSdk.Me.FollowedSites.@Ref.@Ref>();
                var requestInfo = CreatePostRequestInformation(model);
                var result = await RequestAdapter.SendAsync<ApiSdk.Me.FollowedSites.@Ref.@Ref>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<ApiSdk.Me.FollowedSites.@Ref.@Ref> PostAsync(ApiSdk.Me.FollowedSites.@Ref.@Ref model, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default, CancellationToken cancellationToken = default) {
            _ = model ?? throw new ArgumentNullException(nameof(model));
            var requestInfo = CreatePostRequestInformation(model, h, o);
            return await RequestAdapter.SendAsync<ApiSdk.Me.FollowedSites.@Ref.@Ref>(requestInfo, responseHandler, cancellationToken);
        }
