...
namespace ApiSdk.DeviceAppManagement.DefaultManagedAppProtections.Item.Apps {
    ...
    public class AppsRequestBuilder {
        ...
        public Command BuildListCommand() {
            ...
            command.Handler = CommandHandler.Create<string, int?, int?, string, string, bool?, object, object, object>(async (defaultManagedAppProtectionId, top, skip, search, filter, count, orderby, select, expand) => {
                ...
                var result = await RequestAdapter.SendAsync<AppsResponse>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<ManagedMobileApp> PostAsync(ManagedMobileApp model, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default) {
