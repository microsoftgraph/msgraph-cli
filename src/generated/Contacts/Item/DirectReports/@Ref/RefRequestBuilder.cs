    ...
    public class RefRequestBuilder {
        ...
        public Command BuildPostCommand() {
            ...
            command.Handler = CommandHandler.Create<string, string>(async (orgContactId, body) => {
                ...
                var model = parseNode.GetObjectValue<ApiSdk.Contacts.Item.DirectReports.@Ref.@Ref>();
                var requestInfo = CreatePostRequestInformation(model);
                if (!String.IsNullOrEmpty(orgContactId)) requestInfo.PathParameters.Add("orgContact_id", orgContactId);
                var result = await RequestAdapter.SendAsync<ApiSdk.Contacts.Item.DirectReports.@Ref.@Ref>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<ApiSdk.Contacts.Item.DirectReports.@Ref.@Ref> PostAsync(ApiSdk.Contacts.Item.DirectReports.@Ref.@Ref model, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default, CancellationToken cancellationToken = default) {
            _ = model ?? throw new ArgumentNullException(nameof(model));
            var requestInfo = CreatePostRequestInformation(model, h, o);
            return await RequestAdapter.SendAsync<ApiSdk.Contacts.Item.DirectReports.@Ref.@Ref>(requestInfo, responseHandler, cancellationToken);
        }
