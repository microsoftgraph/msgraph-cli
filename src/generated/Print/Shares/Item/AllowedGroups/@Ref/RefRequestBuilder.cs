    ...
    public class RefRequestBuilder {
        ...
        public Command BuildPostCommand() {
            ...
            command.Handler = CommandHandler.Create<string, string>(async (printerShareId, body) => {
                ...
                if (!String.IsNullOrEmpty(printerShareId)) requestInfo.PathParameters.Add("printerShare_id", printerShareId);
                var result = await RequestAdapter.SendAsync<ApiSdk.Print.Shares.Item.AllowedGroups.@Ref.@Ref>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<ApiSdk.Print.Shares.Item.AllowedGroups.@Ref.@Ref> PostAsync(ApiSdk.Print.Shares.Item.AllowedGroups.@Ref.@Ref model, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default) {
