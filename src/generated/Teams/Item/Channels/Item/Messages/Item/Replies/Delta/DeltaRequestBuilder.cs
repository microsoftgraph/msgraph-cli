...
namespace ApiSdk.Teams.Item.Channels.Item.Messages.Item.Replies.Delta {
    ...
    public class DeltaRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.SetHandler(async (string teamId, string channelId, string chatMessageId, FormatterType output, IOutputFormatterFactory outputFormatterFactory, IConsole console) => {
                var requestInfo = CreateGetRequestInformation(q => {
                });
                var response = await RequestAdapter.SendPrimitiveAsync<Stream>(requestInfo);
                var formatter = outputFormatterFactory.GetFormatter(output);
                formatter.WriteOutput(response, console);
            }, teamIdOption, channelIdOption, chatMessageIdOption, outputOption, new OutputFormatterFactoryBinder());
            return command;
        }
        /// <summary>
