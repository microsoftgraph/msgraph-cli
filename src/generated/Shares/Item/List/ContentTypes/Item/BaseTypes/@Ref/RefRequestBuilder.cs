...
namespace ApiSdk.Shares.Item.List.ContentTypes.Item.BaseTypes.@Ref {
    ...
    public class RefRequestBuilder {
        ...
        public Command BuildGetCommand() {
            ...
            command.Handler = CommandHandler.Create<string, string, int?, int?, string, string, bool?, object>(async (sharedDriveItemId, contentTypeId, top, skip, search, filter, count, orderby) => {
        ...
        public Command BuildPostCommand() {
            ...
            command.Handler = CommandHandler.Create<string, string, string>(async (sharedDriveItemId, contentTypeId, body) => {
                ...
                if (!String.IsNullOrEmpty(sharedDriveItemId)) requestInfo.PathParameters.Add("sharedDriveItem_id", sharedDriveItemId);
                if (!String.IsNullOrEmpty(contentTypeId)) requestInfo.PathParameters.Add("contentType_id", contentTypeId);
                var result = await RequestAdapter.SendAsync<ApiSdk.Shares.Item.List.ContentTypes.Item.BaseTypes.@Ref.@Ref>(requestInfo);
                // Print request output. What if the request has no return?
                using var serializer = RequestAdapter.SerializationWriterFactory.GetSerializationWriter("application/json");
                serializer.WriteObjectValue(null, result);
                using var content = serializer.GetSerializedContent();
        ...
        /// <param name="responseHandler">Response handler to use in place of the default response handling provided by the core service</param>
        /// </summary>
        public async Task<ApiSdk.Shares.Item.List.ContentTypes.Item.BaseTypes.@Ref.@Ref> PostAsync(ApiSdk.Shares.Item.List.ContentTypes.Item.BaseTypes.@Ref.@Ref model, Action<IDictionary<string, string>> h = default, IEnumerable<IRequestOption> o = default, IResponseHandler responseHandler = default) {
            _ = model ?? throw new ArgumentNullException(nameof(model));
            var requestInfo = CreatePostRequestInformation(model, h, o);
            return await RequestAdapter.SendAsync<ApiSdk.Shares.Item.List.ContentTypes.Item.BaseTypes.@Ref.@Ref>(requestInfo, responseHandler);
        }
